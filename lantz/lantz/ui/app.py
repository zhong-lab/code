...
class Frontend(QtWidgets.QMainWindow, metaclass=_FrontendType):
...
def start_gui_app(backend, frontend_class, qapp_or_args=None):
    if isinstance(qapp_or_args, QtWidgets.QApplication):
        qapp = qapp_or_args
    else:
        qapp = QtWidgets.QApplication(qapp_or_args or [''])
