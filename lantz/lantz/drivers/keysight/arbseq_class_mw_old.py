...
class PulseGeneratorAWG(Spyrelet):
	...
	def startpulse(self):
		...
		repeatmag1=params['dc repeat unit large'].magnitude
...
# Pi/2 pulse
		...
		pi2PulseI = Arbseq_Class_MW('pi2PulseI', timestep,Wavepi2pulse,PiAmp*Amp_factor_pi2,pulsewidth,freq,phase)
		pi2PulseI.sendTrigger()
		pi2PulseI.create_envelope()

		pi2PulseQ = Arbseq_Class_MW('pi2PulseQ', timestep,Wavepi2pulse,PiAmp*Amp_factor_pi2,pulsewidth,freq,phase+deltaphiiq)
		pi2PulseQ.create_envelope()

# Delay of Tau

		delay1 = Arbseq_Class_MW('delay1', timestep,'DC',0,repeatmag,0,0)
		repeatwidthdelay1=(tau-1.0*pulsewidth)
...
# Delay of 2 tau
		...
		repeatwidthdelay2=(2*tau-pulsewidth)
...
# Sleeptime
	...
	#    ('arbname', {'type': str, 'default': 'arbitrary_name'}),,
		('trigger delay', {'type': float, 'default': 100e-9, 'units':'s'}),	
		('pulse width', {'type': float, 'default': 4e-6, 'units':'s'}),
		('period', {'type': float, 'default': 1, 'units':'s'}),
		('dc repeat unit', {'type': float, 'default': 100e-9, 'units':'s'}),
		...
		('Frequency', {'type': float, 'default': 1e6, 'units':'dimensionless'}),
