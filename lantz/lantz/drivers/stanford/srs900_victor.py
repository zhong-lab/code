...
class SRS900(MessageBasedDriver):
    ...
    def SIM960_set_on_off_proportional(self,key):
      """ turning proportional action on/off
          key: OFF 0, ON 1
      """
      self.flush()
      self.flushoutput()
      self.wait_time(100)
      self.write('SNDT 3, "PCTL {}"'.format(key))

    def SIM960_get_on_off_proportional(self,key):
      """ getting state of proportional action on/off
          key: OFF 0, ON 1
      """
      self.flush()
      self.flushoutput()
      self.wait_time(100)
      self.write('SNDT 3, "PCTL?"')
      msg = self.query('GETN? 3, 80', delay=2)
...
if __name__ == '__main__':
    ...
    with SRS900('GPIB0::2::INSTR') as inst:
      ...
      inst.clear_status()
      v2=inst.SIM960_set_on_off_proportional(1)
