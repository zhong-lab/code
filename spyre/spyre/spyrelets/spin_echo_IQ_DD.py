...
# Long delay for setting the trigger rate 	
		...
		repeatwidthlongdelay = ((trigperiod- 2*(repeatwidthdelay1) - pulsewidth1 - npulses*(4*pulsewidth2) - 4*repeatwidthdelay2 ))  # For XY4
...
# Output On
		...
		self.delaygen.delay['B']=pulsewidth1*0.5+tau+(4*npulses-1)*(2*tau) + pulsewidth2*0.5 + 5e-6  # For XY4
		...
		self.osc.delay_time(pulsewidth1*0.5-50e-6+(4*npulses)*(2*tau))  # For XY4
