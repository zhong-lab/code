...
# Output On
		...
		x = x-x.min()
		y = np.array(y)
		np.savetxt('D:/MW data/20200606/Optimization/sweepPulse1/ch3/{}.txt'.format(Pi2Amp), np.c_[x,y])
		...
		x = x-x.min()
		y = np.array(y)
		np.savetxt('D:/MW data/20200606/Optimization/sweepPulse1/ch4/{}.txt'.format(Pi2Amp), np.c_[x,y])
	...
	def SweepPulse1(self):
		...
		self.osc.setmode('sample')
		self.source.RF_OFF()
		for x in np.linspace(0.1,0.001,50):
			self.record(x)
		self.source.RF_OFF()	

	@SweepPulse1.initializer
	def initialize(self):
		return

	@SweepPulse1.finalizer
	def finalize(self):
		return
