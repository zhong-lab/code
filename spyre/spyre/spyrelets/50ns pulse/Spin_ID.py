...
# Set the Source frequency offset from the cavity by IF frequency 
		...
		self.osc.delay_time(2*tau)  # This makes sure that echo is at center of screen
...
# Start collecting data
		...
		x = x-x.min()
		y = np.array(y)
		np.savetxt('D:/MW data/20200806/ID/Scan1/ch3/{}_{}.txt'.format(PiAmp,tau*1e6), np.c_[x,y])   
		...
		x = x-x.min()
		y = np.array(y)
		np.savetxt('D:/MW data/20200806/ID/Scan1/ch4/{}_{}.txt'.format(PiAmp,tau*1e6), np.c_[x,y])
	...
	def Record_T2(self):
		...
		tauarray=[1e-6,10e-6,25e-6,50e-6,75e-6,100e-6,150e-6,200e-6,250e-6,300e-6,400e-6]
		# PiAmparray=[0.48,0.54,0.66,0.74,0.82,1.0]         #Chosen based on pi-pulse amplitude sweep
		PiAmparray=[0.4]         #second round

		for amp in PiAmparray:
			for tau in tauarray:				
				self.record(tau,amp)
