...
class TwoPulsePhotonEcho(Spyrelet):
	...
	def createHistogram(self,stoparray, timebase, bincount, totalWidth, tau):
		...
		for stoptime in stoparray:
			...
			if binNumber >= bincount:
				...
				hist[binNumber]+=1
		out_name = "D:\\Data\\5.19.2019\\2Pulse_T2_test"  
		np.savez(os.path.join(out_name,self.exp_parameters.widget.get()['File Name']+str(tau)),hist)
	...
	def startpulse(self, timestep=1e-9):
		...
		for i in range(int((params['stop tau']-params['start tau'])/params['step tau'])+1):
			...
			chn2dc1.markerloc = 0
			chn2dc1repeats = int((tau.magnitude-pulse_width)/repeat_unit)
			...
			for j in range(self.exp_parameters.widget.get()['# of Passes']):
				...
				if histCounter%30==0:
					...
					if self.srs.SIM928_voltage[???] >= qunech threshold and quenchCounter<=10:
						...
						continue
					'''
			self.createHistogram(stoparray, timebase, bincount,qutagparams['Total Hist Width Multiplier']*tau.magnitude,tau.magnitude)
	...
	#    ('arbname', {'type': str, 'default': 'arbitrary_name'}),,
		...
		('pulse width', {'type': float, 'default': 2500e-9, 'units':'s'}),
		...
		('shutter offset', {'type': float, 'default': 250e-9, 'units':'s'}),
		('buffer time', {'type': float, 'default': 1000e-6, 'units':'s'}),
		]
		w = ParamWidget(params)
		return w
