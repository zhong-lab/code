...
class ALIGNMENT(Spyrelet):
	...
	def ReflectionDistribution(self):
		...
		for zpoint in range(len(self.zpositions)):
			...
			for xpoint in range(500):
				pw[zpoint][xpoint]=self.powermeter.power.magnitude*1000
				time.sleep(1e-3)		
			self.attocube.stop()
			time.sleep(1)
	...
	def scan_parameters(self):
		...
		('x Voltage', {'type': float, 'default': 30, 'units':'V'}),
		('Frequency', {'type': float, 'default': 250, 'units':'Hz'})
		]
	...
	def initialize(self):
		...
		self.pw = np.zeros((len(self.zpositions),500),dtype=float)
