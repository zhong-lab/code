...
class ALIGNMENT(Spyrelet):
	...
	def ReflectionDistribution(self):
		...
		while i<self.z_steps: # the attocube moves to a set of positions
			...
			# scan along the x axis for this row
			j = 0
	...
	##  a task that scans along the x axis
	@Task(name='Scan X')
	def FFPClinescan(self):

		# set the frequency and voltage for the x axis
		self.attocube.frequency[self.axis_index_x]=Q_(self.jogf,'Hz')
		self.attocube.amplitude[self.axis_index_x]=Q_(self.jogv,'V')

		scanXparams = self.scan_parameters.widget.get()

		foldername = scanXparams['Folder Name']
		filename = scanXparams['File Name']
		...
		PATH = "E:\\Data\\4.4.2022_ffpc_scan\\" + foldername
		print('here')
		print('PATH: '+str(PATH))
		if PATH!="E:\\Data\\4.4.2022_ffpc_scan\\":
		...
		#self.F = open(self.filename+'.csv', 'w')

		self.pw = np.zeros((self.x_points, 3)) # make an empty array

		#while i < self.x_steps: # the attocube moves to a set of positions
		# the attocube moves to the starting x position and overshoots by
		# -150um. Attocube says this must be at least 100um to make the
		# movement repeatable.
		#self.attocube.absolute_move(self.axis_index_x, self.x_start-150)
		...
		# Then the attocube drives to the starting point.
		self.attocube.absolute_move(self.axis_index_x, self.x_start-100)
		time.sleep(1) # sleep time to get feedback from the closed loop
		self.attocube.absolute_move(self.axis_index_x, self.x_start)
		time.sleep(1) # sleep time to get feedback from the closed loop
		...
		i = 0
		while i < self.x_points:
			time.sleep(1)
			# drive the x axis by single step & record the corresponding power reading from the power meter
			j = 0
			while j < self.x_steps:
				self.attocube.single_step(self.axis_index_x, 1)
				j = j+1
			...
			time.sleep(0.5)
			self.pw[i, 0] = i
			self.pw[i, 1] = self.attocube.position[self.axis_index_x].magnitude
			self.pw[i, 2] = p
			...
			# print some data to the console about scan progress
			#print("%f:%f:%f"%(self.x_steps[i],self.attocube.position[self.axis_index_x][i].magnitude,self.pw[i]))

			# write the power array to the reflection distribution variable
			#self.F.write('\n')
			values = {
					'power': self.pw[i, 2],
					'position': self.pw[i, 1],
					'step': self.pw[i, 0],

				}
			self.FFPClinescan.acquire(values)
			# power.append(self.pmd.power.magnitude*1000)
			# position.append(self.attocube.position[self.axis_index_x].magnitude)
			# step.append(i)
			
			i = i + 1
		np.savez(os.path.join(PATH, filename), self.pw[:, 0], self.pw[:, 1], self.pw[:, 2])
		#self.F.close()
	...
	def Position(self):
		while True:
			...
			# acquiring the "values" for the real time plot
			values = {
				'x': self.x,
				'y': self.y,
				'z': self.z,
	...
	def scan_parameters(self):
		...
		params = [
		#('File name', {'type': str, 'default': 'E:\\Data\\ffpc_alignment'}),
		('X start', {'type': float, 'default': 3000*1e-6, 'units':'m'}),
		('X points', {'type': float, 'default': 10}),
		('X steps', {'type': float, 'default': 10}),
		#('Z start', {'type': float, 'default': 2500*1e-6, 'units':'m'}),
		#('Z steps', {'type': float, 'default': 50}),
		('Step Voltage', {'type': float, 'default': 30, 'units':'V'}),
	...
	# the 1D plot widget is used for the power vs. position plot
	@Element(name='ffpclinescan plot')
	def ffpclinescan(self):
		p = LinePlotWidget()
		p.plot('FFPC Reflection')
		return p

	# more code for the power vs. position plot
	@ffpclinescan.on(FFPClinescan.acquired)
	def _plot1d_update(self, ev):
		w = ev.widget
		xs = np.array(self.pw[:,0])
		ys = np.array(self.pw[:,1])
		# if (len(xs)>len(ys)):
		# 	xs = xs[:len(ys)]
		w.set('FFPC Reflection', xs=xs, ys=ys)
		return
	...
	def initialize(self):
		...
		self.x_start = fieldValues['X start'].magnitude*1e6
		#self.z_start = fieldValues['Z start'].magnitude*1e6
		self.foldername = fieldValues['Folder Name']
		self.filename = fieldValues['File Name']
		self.x_points = int(fieldValues['X points'])
		self.x_steps = int(fieldValues['X steps'])
		
		#self.pw = np.zeros((self.x_steps, 3), dtype=float)
