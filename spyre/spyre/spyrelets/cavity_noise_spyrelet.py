...
class Record(Spyrelet):
    ...
    def noise(self,value):
        ...
        frequency_center = 5088.58 * MHz

        self.source.power = -20
        self.source.output = 1
        self.source.Trigger_Setting = 9
        self.source.Ext_FM_Type = 1
        self.source.Ext_FM_Deviation = 1200000 * Hz
        self.lockin.Internal_Frequency = value * Hz
        self.source.frequency = frequency_center
        

        time.sleep(5)
        time0 = self.lockin.System_Time_Hours*3600+self.lockin.System_Time_Min*60+self.lockin.System_Time_Seconds

        t = 0

        with open('D:/MW data/20191106/noise/4/{}.txt'.format(value),'a') as file:
            write_str='%f %f %f %f %f\n'%(0,0,0,0,0)
        ...
        while(t<3600):
            ...
            a = np.loadtxt('D:/MW data/20191106/noise/4/{}.txt'.format(value))
            self.X_s = a[1:,0]
            self.Y_s = a[1:,1]
            self.R_s = a[1:,2]
            self.theta_s = a[1:,3]
            ...
            time.sleep(1)
            t = (self.lockin.System_Time_Hours*3600+self.lockin.System_Time_Min*60+self.lockin.System_Time_Seconds) - time0
        return


    @Task()
    def Record_data_time(self):
        for D in range(1000,1100,1000):
            self.noise(D)
        return
