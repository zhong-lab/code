...
class Record(Spyrelet):
	...
	def record(self,tau,ndelay,count):
# Set the AWG 
		self.dataset.clear()
		...
		Amp_factor_pi2=params['Pi2voltage'].magnitude*0.3333
		Amp_factor_pi=params['Pivoltage'].magnitude*0.3333
...
# Start collecting data
		...
		np.savetxt('D:/MW data/20201106/customized72/Scan2/ch3/{}_{}.{}.txt'.format(tau*1e6,npulses,count), np.c_[xdata,ydata])   
		np.savetxt('D:/MW data/20201106/customized72/Scan2/ch4/{}_{}.{}.txt'.format(tau*1e6,npulses,count), np.c_[xdata1,ydata1])
	...
	def Record_customized72(self):
		...
		for tau in tauarray:
			...
			for count in range(0,numrepeat):
				...
				self.record(tau,ndelay,count)



		return

	@Record_customized72.initializer
	def initialize(self):
		return

	@Record_customized72.finalizer
	def finalize(self):
		return
	...
	#    ('arbname', {'type': str, 'default': 'arbitrary_name'}),,
		...
		('nPulses', {'type': int, 'default': 6, 'units':'dimensionless'}),
		...
		('pulse width', {'type': float, 'default':3e-6, 'units':'s'}),
