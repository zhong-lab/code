...
class ALIGNMENT(Spyrelet):
    ...
    def ReflectionDistribution(self):
        ...
        i = 0
        for zpoint in self.zpositions: # the attocube moves to a set of positions

            # set the frequency and voltage for the x axis
            self.attocube.frequency[self.axis_index_x]=Q_(self.movef,'Hz')
            self.attocube.amplitude[self.axis_index_x]=Q_(self.movev,'V')

            # the Attocube changes which row it drives on
            # each time it overshoots the correct z position by 150um and drives
            # back for better accuracy
            #self.attocube.single_step(self.axis_index_z,+1)
            self.attocube.absolute_move(self.axis_index_z,zpoint+150)
            time.sleep(0.1)
            self.attocube.absolute_move(self.axis_index_z,zpoint)
            time.sleep(0.1)
            ...
            time.sleep(0.1) # sleep time to get feedback from the closed loop

            # TODO: not sure what the following code is supposed to do
            """
            ...
            for xpoint in self.xpositions:
                ...
                time.sleep(0.5) # allow the closed loop movement to complete
                ...
                time.sleep(0.5)
                p4=self.pmd.power.magnitude*1000
                self.pw[i,j]=(p1+p2+p3+p4)/4
                j=j+1 # increment the array index for the x coordinate
            i = i+1 # increment the array index for the z coordinate
            time.sleep(0.1) # allow the closed loop movement to complete
    ...
    def scan_parameters(self):
        ...
        # set the default x step to 2um. This is the minimum repeatable step
        # the scan range default for each axis is 10um
        ...
        ('X step', {'type': float, 'default': 2*1e-6,'units':'m'}),
        ('Z start', {'type': float, 'default': 2500*1e-6, 'units':'m'}),
        ('Z step', {'type': float, 'default': 2*1e-6,'units':'m'}),
    ...
    def initialize(self):
        ...
        self.x_steps=int(fieldValues['X range'].magnitude/fieldValues['X step'].magnitude)
        self.z_steps=int(fieldValues['Z range'].magnitude/fieldValues['Z step'].magnitude)

        # the x positions are linearly spaced points from the start until the
        # end spaced by 2um
        self.xpositions = np.arange(self.x_start,self.x_start+self.x_range,self.x_step)
        self.zpositions = np.arange(self.z_start,self.z_start+self.z_range,self.z_step)
