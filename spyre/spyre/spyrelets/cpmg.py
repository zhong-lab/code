...
class CPMG(Spyrelet):
	...
	def startpulse(self, timestep=1e-9):
		...
		for i in range(int((params['stop tau']-params['start tau'])/params['step tau'])+1):
			...
			self.fungen.create_arbseq('twoPulse', seq, 2)
			...
			# self.fungen.voltage[2] = 2
			# self.fungen.offset[2] = 1
			...
			x = x-x.min()
			y = np.array(y)
			np.savetxt('D:/MW data/20200205/Test11_2usPi/Tau_{}_N_{}.txt'.format(tau*1e6,npulses), np.c_[x,y])   
