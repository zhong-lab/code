...
from lantz.drivers.siglent import SDG2122X
...
class PLThinFilm(Spyrelet):
	...
	def stabilize_EOM(self,timebase,stoparray_i):
		...
		def pulseCounts(self):
			""" sums up the counts in the shutter pulse every half milisecond.
			"""
			stoparray = []
			...
			stoptimes=[i*timebase for i in stoparray]
			pulseSum=len(stoptimes)
			return pulseSum
		...
		qutagparams = self.qutag_params.widget.get()
		EOMcheck=qutagparams['EOM modulator?']
		channel_bias=qutagparams['DC bias channel']
		t_cal=qutagparams['EOM calibration time']
		sensitivity=qutagparams['EOM sensitivity']

		sign=1 # the first attempt to adjust the RF will be positive

		# if the EOM shutter is used
		if EOMcheck==True:
			print('Stabilizing EOM vs. temperature drift')

			# get the initial sum of counts during the pulse
			pulseSum_i=pulseCounts()
			pulseSum_e=pulseCounts() # get the change in counts after 0.5ms

			di=abs(pulseSum_e-pulseSum_i) # get the difference between the two

			t_st=time.time()
			t_curr=t_st
			dt=t_curr-t_st

			while (dt<t_cal.magnitude):
				pulseSum_l=pulseCounts() # check counts

				dl=abs(pulseSum_l-pulseSum_e) # get the difference from the last
				print('time: '+str(dt)+', Change in counts: '+str(dl))

				if dl>sensitivity:
					if not compareDelta(di,dl): # if the difference is greater
						sign*=-1 # flip the sign of the voltage adjustment

					# change the RF amplitude
					newAmp=RFamp+sign*0.001 # the smallest adjustment of siglent
					print('newRF amplitude: '+str(newAmp))

					# check if at the max DC bias or min DC bias allowed by AWG
					if ((self.checkMinMax(RFamp)==True):
						print('Reached limits of AWG voltage')
						sign*=-1 # change the sign of the voltage adjustment
						newAmp=RFamp+sign*0.001 # reverse the last change

					# set the new amplitude on the AWG
					self.siglent.DCvoltage(channel_bias,newAmp)

					pulseSum_e=pulseSum_l # store the last counts
					di=dl	# store the last change
					dt=time.time()-t_st # get the time elapsed
		else:
			print('no EOM used')
		return
	...
	def qutag_params(self):
		"""
		if the EOM shutter check box is checked then the EOM DC voltage will be
		stabilized during the experiment.

		Uses the Siglent AWG for the DC bias control of the shutter.
	...
	#    ('arbname', {'type': str, 'default': 'arbitrary_name'}),,
		...
		('EOM modulator?',{'type':bool}),
		('DC bias channel',{'type':int,'default':1}),
		('DC bias',{'type':float,'default':0}),
		('EOM calibration time',{'type':int,'default':3600,'units':'s'})
		('EOM sensitivity',{'type':int,'default':1000})
		]
		w = ParamWidget(params)
		return w
