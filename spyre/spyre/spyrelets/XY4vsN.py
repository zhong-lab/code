...
# Long delay for setting the trigger rate 	
		...
		repeatwidthlongdelay = ((trigperiod- 2*(repeatwidthdelay1) - pulsewidth1 - npulses*(4*pulsewidth2) - 4*repeatwidthdelay2 ))
...
# Set the delay generator for gating 
		...
		self.delaygen.delay['B']=pulsewidth1*0.5+tau+(4*npulses-1)*(2*tau) + pulsewidth2*0.5 + 5e-6
...
# Set the Source frequency offset from the cavity by IF frequency 
		...
		self.osc.delay_time(pulsewidth1*0.5-50e-6+(4*npulses)*(2*tau)) 
...
# Start collecting data
		...
		while(measurementno<nmeasurement):
			...
			x = x-x.min()
			y = np.array(y)
			np.savetxt('D:/MW data/20200614/XY4/Scan3/ch3/{}/{}_{}.txt'.format(measurementno,tau*1e6,npulses), np.c_[x,y])   
			...
			x = x-x.min()
			y = np.array(y)
			np.savetxt('D:/MW data/20200614/XY4/Scan3/ch4/{}/{}_{}.txt'.format(measurementno,tau*1e6,npulses), np.c_[x,y])
	...
	@Record_XY4.initializer
	def initialize(self):
		return

	@Record_XY4.finalizer
	def finalize(self):
		return
