...
# Make sequence
		...
		seq = [delay0I, piPulseI]

		seq1 = [delay0Q, piPulseQ]



		self.fungen.create_arbseq('onePulseI', seq, 1)
		self.fungen.create_arbseq('onePulseQ',seq1,2)
...
# Set the delay generator for triggering the AWG and scope
		...
		self.delaygen.delay['E']=0+pulsewidth+tau_wait
...
# Set the Source frequency offset from the cavity by IF frequency 
		...
		self.osc.delay_time(pulsewidth-50e-6)  # This makes sure that echo is at center of screen
...
# Start collecting data with RF on
		...
		x = x-x.min()
		y = np.array(y)
		np.savetxt('D:/MW data/20201026/ODMR/Scan3/RFON/{}.txt'.format(freqOpt), np.c_[x,y])   

		peak2peak=self.osc.measure_pk2pk(2)


		file = open('D:/MW data/20201026/ODMR/Scan3/RFON/data.txt','a') 
		write_str='{}\t{}'.format(freqOpt,peak2peak)
		file.write(write_str)
		file.close()



# Start collecting data with RF oFF
		self.fungen.output[1] = 'OFF'
		self.fungen.output[2] = 'OFF'
		self.source.RF_OFF()

		self.osc.setmode('sample')
		time.sleep(60)  # Wait 1 min between the RF on and RF off experiment for the spins to recover
		...
		x = x-x.min()
		y = np.array(y)
		np.savetxt('D:/MW data/20201026/ODMR/Scan3/RFOFF/{}.txt'.format(freqOpt), np.c_[x,y])   

		peak2peak=self.osc.measure_pk2pk(2)


		file = open('D:/MW data/20201026/ODMR/Scan3/RFOFF/data.txt','a') 
		write_str='{}\t{}'.format(freqOpt,peak2peak)
		file.write(write_str)
		file.close()
	...
	#    ('arbname', {'type': str, 'default': 'arbitrary_name'}),,
		...
		('trigger delay', {'type': float, 'default': 320e-9, 'units':'s'}),	
		('timestep', {'type': float, 'default': 1e-8, 'units':'s'}),
		('period', {'type': float, 'default': 0.1, 'units':'s'}),
		('nAverage', {'type': int, 'default': 500, 'units':'dimensionless'}),
		...
		('Optpulsewidth', {'type': float, 'default': 20.0e-6, 'units':'s'}),
		('Waitingtime', {'type': float, 'default': 1.0e-6, 'units':'s'}),	
		('FreqOPT', {'type': float, 'default': 195227.0, 'units':'dimensionless'}),
