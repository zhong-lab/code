...
# Set the Source frequency offset from the cavity by IF frequency 
		...
		self.osc.delay_time(pulsewidth1*0.5-50e-6+(npulses)*(2*tau))  # This makes sure that echo is at center of screen
...
# Start collecting data
		...
		while(measurementno<nmeasurement):
			...
			x = x-x.min()
			y = np.array(y)
			np.savetxt('D:/MW data/20200614/CPMG/tau50/ch3/{}/{}.txt'.format(measurementno,npulses), np.c_[x,y])   
			...
			x = x-x.min()
			y = np.array(y)
			np.savetxt('D:/MW data/20200614/CPMG/tau50/ch4/{}/{}.txt'.format(measurementno,npulses), np.c_[x,y])
	...
	def Record_CPMG(self):
		...
		taurange=params['tauRange'].magnitude
		npoints=params['nPoints'].magnitude
		npulses=int(np.round(taurange/(2*tau)))


		for x in np.linspace(1,npulses+1,npoints):
			self.record(tau,int(np.round(x)))
		return
	...
	#    ('arbname', {'type': str, 'default': 'arbitrary_name'}),,
		...
		('tauRange', {'type': float, 'default': 24e-3, 'units':'s'}),
