...
# Output On
		...
		x = x-x.min()
		y = np.array(y)
		np.savetxt('D:/MW data/20200606/Optimization/sweepPulse2/ch3/{}.txt'.format(PiAmp), np.c_[x,y])
		...
		x = x-x.min()
		y = np.array(y)
		np.savetxt('D:/MW data/20200606/Optimization/sweepPulse2/ch4/{}.txt'.format(PiAmp), np.c_[x,y])
	...
	def SweepPulse2(self):
		...
		self.osc.setmode('sample')
		self.source.RF_OFF()
		for x in np.linspace(0.001,0.5,50):
			self.record(x)
		self.source.RF_OFF()	
