...
# PiTh Pulse
		...
		piThPulseI = Arbseq_Class_MW('piThPulseI', timestep,Wavepipulse,PiAmp,pulsewidth2,freq,phase+deltaphase3)
		piThPulseI.create_envelope()

		piThPulseQ = Arbseq_Class_MW('piThPulseQ', timestep,Wavepipulse,PiAmp,pulsewidth2,freq,phase+deltaphase3+deltaphiiq)
...
# Long delay for setting the trigger rate 	
		...
		repeatwidthlongdelay = ((trigperiod- 2*(repeatwidthdelay1) - pulsewidth1 - npulses*(10*pulsewidth2) - 10*repeatwidthdelay2 ))
...
# Set the delay generator for gating 
		...
		self.delaygen.delay['B']=pulsewidth1*0.5+tau+(10*npulses-1)*(2*tau) + pulsewidth2*0.5 + 5e-6
...
# Set the Source frequency offset from the cavity by IF frequency 
		...
		self.osc.delay_time(pulsewidth1*0.5-50e-6+(10*npulses)*(2*tau)) 
...
# Start collecting data
		...
		while(measurementno<nmeasurement):
			...
			x = x-x.min()
			y = np.array(y)
			np.savetxt('D:/MW data/20200614/KDDx/Scan1/ch3/{}/{}_{}.txt'.format(measurementno,tau*1e6,npulses), np.c_[x,y])   
			...
			x = x-x.min()
			y = np.array(y)
			np.savetxt('D:/MW data/20200614/KDDx/Scan1/ch4/{}/{}_{}.txt'.format(measurementno,tau*1e6,npulses), np.c_[x,y])
