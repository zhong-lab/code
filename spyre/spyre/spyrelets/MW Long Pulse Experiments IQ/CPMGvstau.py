...
# Start collecting data
		...
		while(measurementno<nmeasurement):
			...
			x = x-x.min()
			y = np.array(y)
			np.savetxt('D:/MW data/20200614/CPMG/cpmgvstau/ch3/{}/{}_{}.txt'.format(measurementno,tau*1e6,npulses), np.c_[x,y])   
			...
			x = x-x.min()
			y = np.array(y)
			np.savetxt('D:/MW data/20200614/CPMG/cpmgvstau/ch4/{}/{}_{}.txt'.format(measurementno,tau*1e6,npulses), np.c_[x,y])
	...
	def Record_CPMG(self):
		...
		for tau in tauarray:
			for x in np.linspace(1,npulses+1,npoints):
				self.record(tau*1e-6,int(np.round(x)))
		return
