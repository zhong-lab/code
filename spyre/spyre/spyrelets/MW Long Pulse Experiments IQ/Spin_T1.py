...
# Pi inversion Pulse
		...
		piinvPulseI = Arbseq_Class_MW('piinvPulseI', timestep,Wavepipulse,PiAmp,pulsewidth2,freq,phase)
		piinvPulseI.create_envelope()
		piinvPulseI.sendTrigger()


		piinvPulseQ = Arbseq_Class_MW('piinvPulseQ', timestep,Wavepipulse,PiAmp,pulsewidth2,freq,phase+deltaphiiq)
		piinvPulseQ.create_envelope()



# Pi Pulse

		piPulseI = Arbseq_Class_MW('piPulseI', timestep,Wavepipulse,PiAmp,pulsewidth2,freq,phase)
		piPulseI.create_envelope()

		piPulseQ = Arbseq_Class_MW('piPulseQ', timestep,Wavepipulse,PiAmp,pulsewidth2,freq,phase+deltaphiiq)
...
# Delay of 2 tau
		...
		delay3 = Arbseq_Class_MW('delay3', timestep,'DC',0,repeatmag,0,0) #Block for 2 tau
		repeatwidthdelay3=(tau1-pulsewidth2)
		delay3.setRepeats(repeatwidthdelay3)
		delay3.create_envelope()
...
# Long delay for setting the trigger rate 	
		...
		repeatwidthlongdelay = ((trigperiod - tau1 - tau - pulsewidth2  ))
		longdelay.setRepeats(repeatwidthlongdelay)
		longdelay.create_envelope()

# Send all the Arbs
		self.fungen.send_arb(piinvPulseI,1)
		self.fungen.send_arb(delay3,1)
		...
		self.fungen.send_arb(piinvPulseQ,2)
		self.fungen.send_arb(delay3,2)
...
# Make sequence
		...
		seq = [delay1, piinvPulseI, delay3, pi2PulseI, delay1, piPulseI]

		seq1 = [delay1, piinvPulseQ, delay3, pi2PulseQ, delay1, piPulseQ]
...
# Set the delay generator for gating 
		...
		self.delaygen.delay['B']= tau1 +pulsewidth1*0.5+tau+ pulsewidth2*0.5+5e-6
...
# Set the Source frequency offset from the cavity by IF frequency 
		...
		# self.osc.delay_time(tau1 + pulsewidth1*0.5+ 2*tau -50e-6)  # This makes sure that echo is at center of screen
		self.osc.delay_time(tau1)  # This makes sure that echo is at center of screen
...
# Start collecting data
		...
		while(measurementno<nmeasurement):
			...
			x = x-x.min()
			y = np.array(y)
			np.savetxt('D:/MW data/20200614/SpinT1/Scan5/ch3/{}/{}.txt'.format(measurementno,tau1*1e6), np.c_[x,y])   
			...
			x = x-x.min()
			y = np.array(y)
			np.savetxt('D:/MW data/20200614/SpinT1/Scan5/ch4/{}/{}.txt'.format(measurementno,tau1*1e6), np.c_[x,y])
	...
	#    ('arbname', {'type': str, 'default': 'arbitrary_name'}),,
		...
		('tau1', {'type': float, 'default': 500e-6, 'units':'s'}),
		('tau2', {'type': float, 'default': 1500e-3, 'units':'s'}),
		('nPoints', {'type': int, 'default': 15, 'units':'dimensionless'}),
		('nAverage', {'type': int, 'default': 300, 'units':'dimensionless'}),
