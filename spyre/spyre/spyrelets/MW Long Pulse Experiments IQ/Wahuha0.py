...
# Pix/2 pulse
		...
		pi2xPulseI = Arbseq_Class_MW('pi2xPulseI', timestep,Wavepi2pulse,Pi2Amp,pulsewidth1,freq,phase)
		pi2xPulseI.create_envelope()

		pi2xPulseQ = Arbseq_Class_MW('pi2xPulseQ', timestep,Wavepi2pulse,Pi2Amp,pulsewidth1,freq,phase+deltaphiiq)
...
# Piy/2 pulse
		...
		pi2yPulseI = Arbseq_Class_MW('pi2yPulseI', timestep,Wavepi2pulse,Pi2Amp,pulsewidth1,freq,phase+90)
		pi2yPulseI.sendTrigger()

		pi2yPulseQ = Arbseq_Class_MW('pi2yPulseQ', timestep,Wavepi2pulse,Pi2Amp,pulsewidth1,freq,phase+deltaphiiq+90)
...
# Pimx/2 pulse
		...
		pi2mxPulseI = Arbseq_Class_MW('pi2mxPulseI', timestep,Wavepi2pulse,Pi2Amp,pulsewidth1,freq,phase+180)
		pixm2PulseI.create_envelope()

		pi2mxPulseQ = Arbseq_Class_MW('pi2mxPulseQ', timestep,Wavepi2pulse,Pi2Amp,pulsewidth1,freq,phase+deltaphiiq+180)
...
# Pimy/2 pulse
		...
		pi2myPulseI = Arbseq_Class_MW('pi2myPulseI', timestep,Wavepi2pulse,Pi2Amp,pulsewidth1,freq,phase+270)
		pi2myPulseI.sendTrigger()

		pi2myPulseQ = Arbseq_Class_MW('pi2myPulseQ', timestep,Wavepi2pulse,Pi2Amp,pulsewidth1,freq,phase+deltaphiiq+270)
...
# Long delay for setting the trigger rate 	
		...
		repeatwidthlongdelay = ((trigperiod- 12*tau1 + pulsewidth1*0.5 + pulsewidth2*0.5 ))
...
# Start collecting data
		...
		while(measurementno<nmeasurement):
			...
			x = x-x.min()
			y = np.array(y)
			np.savetxt('D:/MW data/20200614/Wahuha/Scan1/ch3/{}/{}.txt'.format(measurementno,tau*1e6), np.c_[x,y])   
			...
			x = x-x.min()
			y = np.array(y)
			np.savetxt('D:/MW data/20200614/Wahuha/Scan1/ch4/{}/{}.txt'.format(measurementno,tau*1e6), np.c_[x,y])
	...
	#    ('arbname', {'type': str, 'default': 'arbitrary_name'}),,
		...
		('tau1', {'type': float, 'default': 10e-6, 'units':'s'}),
		('tau2', {'type': float, 'default': 100e-6, 'units':'s'}),
