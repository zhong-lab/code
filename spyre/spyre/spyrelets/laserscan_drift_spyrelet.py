...
#### The integration time means the time period for averaging the data from the power meter
...
class LaserScan(Spyrelet):
	...
	def scan(self):
		...
		integration_time=param['Integrationtime'].magnitude
		...
		with Client(self.conn1) as dlc:
			for x in range(n):
				...
				for item in self.wv:
					...
					powers=[]
					starttime = time.time()
					while time.time()-starttime<integration_time:
						powers.append(self.pmd.power.magnitude*1e9)
						#print(powers)
					xx.append(np.mean(powers))
					print(len(xx))
