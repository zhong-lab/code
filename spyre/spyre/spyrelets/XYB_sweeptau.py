...
# Set the Source frequency offset from the cavity by IF frequency 
		...
		self.osc.delay_time(48*2*tau*npulses-800e-9)  # This makes sure that echo is at center of screen
...
# Start collecting data
		...
		x = x-x.min()
		y = np.array(y)
		np.savetxt('D:/MW data/20201026/XYB/Scan1/ch3/{}.txt'.format(48*2*tau*npulses*1e6), np.c_[x,y])   
		...
		x = x-x.min()
		y = np.array(y)
		np.savetxt('D:/MW data/20201026/XYB/Scan1/ch4/{}.txt'.format(48*2*tau*npulses*1e6), np.c_[x,y])
	...
	def Record_XYB(self):
		...
		tauarray=[5e-6,7e-6,7.5e-6,10e-6,15e-6,16e-6,20e-6,40e-6,50e-6,75e-6,100e-6,125e-6,150e-6,175e-6,200e-6]
