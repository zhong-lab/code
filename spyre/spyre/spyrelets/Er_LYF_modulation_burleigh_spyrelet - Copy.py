...
class SweepSignal(Spyrelet):
    ...
    def sweep_frequency(self):
        ...
        carrier_list=np.arange(1530.2725, 1530.3084, 0.001)
        ...
        fr_low = [fr_low4, fr_low2, fr_low3, fr_low1, fr_low5]
        fr_high = [fr_high4, fr_high2, fr_high3, fr_high1, fr_high5]

        # fr_low = [fr_low4, fr_low2]
        # fr_high =[fr_high4, fr_high2]
        ...
        for carrier in carrier_list:
            carrier_wl=round(carrier, 4)
            for segment in zip(fr_low, fr_high):
                self.analyzer.freq_star = segment[0]
                time.sleep(5)
                self.analyzer.freq_stop = segment[1]
                time.sleep(25)
                for i in range(num_scan):
                    print('This is scan #{} on freq range {} - {} for ref at {}'.format(i, segment[0], segment[1], carrier_wl))
                    ...
                    if iter <= 30:
                        ...
                        self.analyzer.savefile(str(carrier_wl) + str(segment[0]) + file_name + curr.strftime("%Y-%m-%d_%H-%M-%S") + "_#" + str(i))
                        # self.analyzer.savefile(str(segment[0]) + file_name + curr.strftime("%Y-%m-%d_%H-%M-%S") + "_#" + str(i) + "_" + str(current_wl))
                        time.sleep(15)
            # print('here 4!')
    ...
    def Laser_Frequency_Settings(self):
        laser_freq_params = [
            ('carrier wavelength (nm)', {'type': float, 'default': 1530.4076}),
    ...
    def Sweep_Frequency_Settings(self):
        sweep_freq_params = [
            ...
            ('stop frequency1', {'type': float, 'default': 2350e6, 'units': 'Hz'}),
            ('start frequency2', {'type': float, 'default': 2350e6, 'units': 'Hz'}),
            ('stop frequency2', {'type': float, 'default': 3150e6, 'units': 'Hz'}),
            ('start frequency3', {'type': float, 'default': 3150e6, 'units': 'Hz'}),
            ('stop frequency3', {'type': float, 'default': 4600e6, 'units': 'Hz'}),
            ('start frequency4', {'type': float, 'default': 4600e6, 'units': 'Hz'}),
            ('stop frequency4', {'type': float, 'default': 5200e6, 'units': 'Hz'}),
            ('start frequency5', {'type': float, 'default': 5200e6, 'units': 'Hz'}),
            ('stop frequency5', {'type': float, 'default': 7100000000, 'units': 'Hz'}),
            ('TG sweep power (dBm)', {'type': float, 'default': -5.5}),
            ('ref level (dBm)', {'type': float, 'default': -10.6}),
            ('Num Scan', {'type': int, 'default': 3}),
            ('File Name', {'type': str, 'default': '_Er_LYF_Modulation_'}),
        ]
