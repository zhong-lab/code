...
class PLThinFilm(Spyrelet):
	...
	def homelaser(self,start):
		...
		with Client(self.laser) as client:
			...
			while current < start-0.002 or current > start+0.002:
	...
	def readlaser(self, lasermeasuretime):
		...
		while looptime-starttime < lasermeasuretime:
			loopstart = time.time()
			laserwl.append(self.wm.measure_wavelength())
			time.sleep(1.2)
	...
	def piezo_scan(self,timestep=100e-9): 
		...
		time.sleep(2)
		print('Current wavelength (motor setting): ' + str(self.wm.measure_wavelength()))
	...
	def startpulse(self, timestep = 100e-9):
		...
		for i in range(points):
			...
			self.homelaser(wlTargets[i])
			print('Laser set to:' + str(wlTargets[i]))
			print('current wavelength: ' + str(self.wm.measure_wavelength()))
			time.sleep(2)
	...
	#    ('arbname', {'type': str, 'default': 'arbitrary_name'}),,
		...
		('start', {'type': float, 'default': 1529}),
		('stop', {'type': float, 'default': 1531}),
	...
	#    ('arbname', {'type': str, 'default': 'arbitrary_name'}),,
		...
		('# of points', {'type': int, 'default': 501}),
		('Measurement Time', {'type': int, 'default': 120, 'units':'s'}),
		('Lasermeasuretime',{'type':float,'default': 10,'units':'s'}), # the measurement time to get he averaged laser wavelength in the beginning
		...
		('AWG Pulse Width',{'type': float,'default': 10e-6,'units':'s'}),
		('File Name', {'type': str}),
		]
		w = ParamWidget(params)
		return w
