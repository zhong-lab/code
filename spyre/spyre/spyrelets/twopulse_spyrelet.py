...
class TwoPulse(Spyrelet):
	...
	def startpulse(self, timestep=1e-9):
		...
		for i in range(int((params['stop tau']-params['start tau'])/params['step tau'])+1):
			tau=tau0.magnitude +i*params['step tau'].magnitude
			...
			chn1dc.markerloc = 0
			chn1dcrepeats = int((tau-1.5*params['pulse width'].magnitude)/(params['repeat unit'].magnitude)) # 1.5
			...
			chn1dc1.markerloc = 0
			chn1dcrepeats1 = int((2*tau-2*params['pulse width'].magnitude)/(params['repeat unit'].magnitude)) # 1.5
			...
			chn1dc2repeats = int((params['period'].magnitude-tau-npulses*(2*tau)-(1+npulses*2)*params['pulse width'].magnitude- 2*chn1dcrepeats1*repeatmag - chn1dcrepeats*repeatmag)/(params['repeat unit'].magnitude))
			...
			# seq = [chn1pulse, chn1dc, chn1pulse2, chn1dc1, chn1pulse2, chn1dc1, chn1pulse2, chn1dc2]
	...
	#    ('arbname', {'type': str, 'default': 'arbitrary_name'}),,
		('pulse width', {'type': float, 'default': 2e-6, 'units':'s'}),
		('period', {'type': float, 'default': 1, 'units':'s'}),
		('repeat unit', {'type': float, 'default': 1e-6, 'units':'s'}),
		('start tau', {'type': float, 'default': 11e-6, 'units':'s'}),
