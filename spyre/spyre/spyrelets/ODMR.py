...
# Start collecting data with RF on
		...
		x = x-x.min()
		y = np.array(y)
		np.savetxt('D:/MW data/20201026/ODMR/Scan2/RFON/{}.txt'.format(freqOpt), np.c_[x,y])   

		peak2peak=self.osc.measure_pk2pk(2)


		file = open('D:/MW data/20201026/ODMR/Scan2/RFON/data.txt','a') 
...
# Start collecting data with RF oFF
		...
		x = x-x.min()
		y = np.array(y)
		np.savetxt('D:/MW data/20201026/ODMR/Scan2/RFOFF/{}.txt'.format(freqOpt), np.c_[x,y])   

		peak2peak=self.osc.measure_pk2pk(2)


		file = open('D:/MW data/20201026/ODMR/Scan2/RFOFF/data.txt','a') 
		write_str='{}\t{}'.format(freqOpt,peak2peak)
		file.write(write_str)
		file.close()
	...
	#    ('arbname', {'type': str, 'default': 'arbitrary_name'}),,
		...
		('Optpulsewidth', {'type': float, 'default': 50.0e-6, 'units':'s'}),
		('Waitingtime', {'type': float, 'default': 10.0e-6, 'units':'s'}),	
		('FreqOPT', {'type': float, 'default': 195228.0, 'units':'dimensionless'}),
