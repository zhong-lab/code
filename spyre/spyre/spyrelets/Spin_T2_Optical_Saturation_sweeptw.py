...
# Start collecting data
		...
		x = x-x.min()
		y = np.array(y)
		np.savetxt('D:/MW data/20201111/OpticalSaturation/Scan9/ch3/{}.txt'.format(tau_wait*1e3), np.c_[x,y])   
		...
		x = x-x.min()
		y = np.array(y)
		np.savetxt('D:/MW data/20201111/OpticalSaturation/Scan9/ch4/{}.txt'.format(tau_wait*1e3), np.c_[x,y])
	...
	def Record_T2(self):
		...
		time.sleep(3600)  #sleep for 1 hours for frige to coolsdown
		...
		tauarray=[0.5e-3,1e-3,5e-3,10e-3,50e-3,100e-3,200e-3,300e-3,400e-3,500e-3,600e-3,750e-3,1]
