...
from spyre.widgets.code_editor import Scintilla_Code_Editor, Monokai_Python_Lexer

from lantz import Q_

from lantz.drivers.stanford.sg396 import SG396

from spyre.spyrelets.onthefly_snippets import snippets, default_snippet 
import re

def build_full_snippet(snippet_name):
    snippet = ''
    for part_name in snippets[snippet_name].keys():
        snippet += "\n#------------ SNIPPET: '{}':{}-----------------\n".format(snippet_name, part_name.ljust(25-len(snippet_name), '-'))
        snippet += snippets[snippet_name][part_name]
        snippet += "\n#--------------------------------------------------------------------\n"
    return snippet

def modify_code(current_code, snippet_name):
    code = current_code
    indent = ' '*4
    snippet = snippets[snippet_name]
    if 'initialize' in snippet.keys():
        code = code.replace('def initialize(self):', 'def initialize(self):\n'+indent+snippet['initialize'])
    if 'finalize' in snippet.keys():
        code = code.replace('def finalize(self):', 'def finalize(self):\n'+indent+snippet['finalize'])
    if 'helper' in snippet.keys():
        code += '\n' + snippet['helper']
    return code
...
class OnTheFlySpyrelet(Spyrelet):
    ...
    def code_editor(self):
        ...
        editor.request_run_signal.connect(lambda: self._sweep())
        return editor

    @Element(name = 'Snippets Menu')
    def snippet_menu(self):
        ...
        lexer = Monokai_Python_Lexer(editor)
        editor.setLexer(lexer)
        update_text = lambda *args: editor.setText(build_full_snippet(snippet_options.currentText()))
        update_text()
        ...
        mod_code = lambda : self.code_editor.widget.setText(modify_code(self.code_editor.widget.text(), snippet_options.currentText()))
