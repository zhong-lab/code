...
# Pi/2 pulse
		...
		pi2PulseI = Arbseq_Class_MW('pi2PulseI', timestep,Wavepi2pulse,Pi2Amp*phasefactor,pulsewidth1,freq,phase)
		pi2PulseI.sendTrigger()
		pi2PulseI.create_envelope()

		pi2PulseQ = Arbseq_Class_MW('pi2PulseQ', timestep,Wavepi2pulse,Pi2Amp*phasefactor,pulsewidth1,freq,phase+deltaphiiq)
...
# Output On
		...
		x = x-x.min()
		y = np.array(y)
		np.savetxt('D:/MW data/20200614/ch3/test1.txt', np.c_[x,y])
	...
	@Record_Spin_Echo_IQ.initializer
	def initialize(self):
		return

	@Record_Spin_Echo_IQ.finalizer
	def finalize(self):
		return
