...
# Set the Source frequency offset from the cavity by IF frequency 
		...
		self.osc.delay_time(36*2*tau*npulses-800e-9)  # This makes sure that echo is at center of screen
...
# Start collecting data
		...
		x = x-x.min()
		y = np.array(y)
		np.savetxt('D:/MW data/20201102/customized72/Scan1/ch3/{}.txt'.format(36*2*tau*npulses*1e6), np.c_[x,y])   
		...
		x = x-x.min()
		y = np.array(y)
		np.savetxt('D:/MW data/20201102/customized72/Scan1/ch4/{}.txt'.format(36*2*tau*npulses*1e6), np.c_[x,y])
