...
class ThreePhotonEcho(Spyrelet):
...
#        tau = params['tau start'].magnitude
    ...
    #    pulses.delays = (2e-6,tau - pulse_width*1.5,0)
        ...
        DC.create_sequence()
        DC.nrepeats = int((totaltime - pulses12.totaltime - wait1.totaltime*wait1.nrepeats - pulses3.totaltime)/tri_time) - 1
...
#        return chn2_shb, freq_rampdown2, chn2, freq_rampup2, heterodyne2, chn2_repeat, end2, chn1_shb, freq_rampdown1, pulses, freq_rampup1, heterodyne1, DC, end1
    ...
    def startpulses(self):
        ...
        tau_stop = params['tau stop'].magnitude
        tau_step = params['tau step'].magnitude
        ...
        writer.writerow(['T_wait', 'Intensity'])
        ...
        while tau <= tau_stop:
            ...
            pulses12, wait1, pulses3, DC, end1 = self.create_pulses(tau)
...
#            chn1 = [chn1_shb, freq_rampdown1, pulses, freq_rampup1, heterodyne1, DC, end1]
            chn1 = [pulses12, wait1, pulses3, DC, end1]
...
#            self.fungen.trigger_source[2] = 'Timer'
 ...
 #           print('chn2 output voltage is:' + str(self.fungen.vpp[2]))
            ...
            tau += tau_step
            print('T_wait changed to {}'.format(tau))
            time.sleep(5)
...
#        self.fungen.output[2] = 'OFF'
    ...
    #    ('arbname', {'type': str, 'default': 'arbitrary_name'}),,
        ...
        ('tri time', {'type': float, 'default': 5e-6, 'units':'s'}),
        ('heterodyne time', {'type': float, 'default': 10e-6, 'units':'s'}),
        ('ramp time', {'type': float, 'default': 2e-6, 'units':'s'}),
        ('wait start', {'type': float, 'default': 5e-6, 'units':'s'}),
        ('wait stop', {'type': float, 'default': 1000e-6, 'units':'s'}),
        ('wait step', {'type': float, 'default': 5e-6, 'units':'s'}),
        ('time step', {'type': float, 'default': 1e-8, 'units':'s'}),
        ('tau start', {'type': float, 'default': 1.5e-6, 'units':'s'}),
        ('tau stop', {'type': float, 'default': 9e-6, 'units':'s'}),
        ('tau step', {'type': float, 'default': 0.5e-6, 'units':'s'}),
        ('voltage step', {'type': float, 'default': 0.0212, 'units':'V'}),
        ('echo voltage', {'type': float, 'default': 0.04, 'units':'V'}),
        ('pulse widths', {'type': float, 'default': 0.5e-6, 'units':'s'}),
