...
from spyre.element import Element_QWidget

class ParamWidget(Element_QWidget):
    ...
    def init_ui(self):
        ...
        getter_methods = dict()
        setter_methods = dict()
        for parameter_name, parameter_opts in self.parameters:
...
if __name__ == '__main__':
    from rangespace import Rangespace
    app = QtWidgets.QApplication([])
    rangespace = Rangespace(unit='GHz')
    parameters = [
        ('Frequency', {
            'type': object,
            'widget': rangespace,
            'getter': lambda: rangespace.array,
        }),
        ('Close on completion', {
            'type': bool,
        }),
    ...
    pw = ParamWidget(parameters)
    pw.show()
    button = QtWidgets.QPushButton('test')
    button.clicked.connect(lambda: print(pw.get()))
    button.show()
    app.exec_()
